...
public class LoginActivity extends Activity {
	...
	public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			//TODO: find better way to manage/pass context
			ApiHelper apihelper = ApiHelper.getInstance(context);
			returnString = apihelper.authenticate(mEmail, mPassword, context);
			...
			if(returnString == null){
				...
				if(user != null){
					...
					user.setApiKey(returnString);
					db.updateUser(user);
				}
				else {
					// user does not exist, add
					Log.d(TAG, "Adding new user....");
					db.addUser(new User(mEmail, returnString));
				}
				return true;
			}
		...
		protected void onPostExecute(final Boolean success) {
			...
			if (success) {
				finish();
				startActivity(new Intent(SUBJECT_ACTIVITY));				
			} else {
				mPasswordView
						.setError(getString(R.string.error_signin_fail));
